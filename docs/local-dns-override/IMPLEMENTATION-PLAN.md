...
Authorization: implementation, GitHub signing and final release are already authorized. Temporary Thor testing, Guard suspension/VPN switching with restoration, and saving the supplied Android provider are now explicitly authorized. Upstream auto-update and current-task monitoring are authorized only after validation. Stage 1 is BLOCKED pending user authentication in TailDNS; Stages 2–7 are NOT STARTED. No implementation stage is complete. Evidence gates remain required work, not passing results.
...
Satisfied: R15 test-install identity, downloaded candidate verification and installation alongside the official Thor app. Remaining: all real Android workflow/routing/lifecycle criteria above and their complete reconciliation. B1's permission prerequisite is resolved. External blocker B2: R01/R05 editor-to-real-query and R06–R08 device acceptance require the user's separate authentication in TailDNS. Resolution: complete interactive login; then perform the authorized controlled VPN switch and required matrix with available test-network/exit-node access. Stage 1 closure and dependent stages cannot be considered complete until these checks pass. Tracked deferrals: none. Android automatic-follow behavior belongs to Stage 2; native Guard-obsolescence work to Stage 3; Windows checks to Stage 4.
