...
Satisfied: Thor initial ordinary-app read, UI-closed change notification and end-to-end propagation, unsupported-input fail-closed behavior with retained MagicDNS, invalid-to-valid recovery without reconnecting, successive-save convergence, live strict-mode conflict reporting, immediate source-switch persistence without a save button, manual-after-follow queries, observer removal on switching to manual, and no observer registration while disconnected. See [ANDROID-VALIDATION.md](ANDROID-VALIDATION.md) for exact candidates and evidence. Remaining: keyboard-Done/manual-invalid-input UI verification (R09), direct follow-enabled disconnect/recreation verification and final reconciliation. Profile isolation, read/registration failure and stale callback contracts have focused host-test evidence, not a second real account on Thor. External blocker B4: the ADB manual-input test command was rejected by tool policy before execution. Resolve through an allowed device-input verification route or user-performed Thor check; do not retry the prohibited action through another shell/tool. Stage 2 closure and integrated validation/release cannot be claimed until its required checks pass. The other remaining checks are ordinary unfinished work, not deferrals. Tracked deferrals: none.
