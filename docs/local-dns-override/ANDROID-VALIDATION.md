...
Specification 1.8 R18 therefore moves the fix to the task-visibility boundary: every TailDNS task-owning activity must set `android:excludeFromRecents="true"`. The pre-fix regression failed for both `MainActivity` and `ShareActivity`; it passed after the two manifest attributes were added. No vendor whitelist mutation, root helper, task-lock dependency or restart watchdog is introduced.

Trusted candidate run [35479401193](https://github.com/Darkaxt/TailDNS/actions/runs/35479401193) passed the source and compiled-manifest exclusion contracts, Android/native tests and the Windows companion build for Android `5284a1b2211ee3258b1b9d94db5263673214f53c` with core `f5de5ace94bb3fb21794d1e10184029709242aa0`. Isolated signing run [35479774581](https://github.com/Darkaxt/TailDNS/actions/runs/35479774581) also passed. Its downloaded validation APK independently matched SHA-256 `edeaa9936217ce180ed5a30273490491a0ca4c8d534146113f6594d412d89cf6`, package `io.github.darkaxt.taildns`, code `298310800`, version `1.103.312-taildns.5`, the pinned signer and v2/v3 signatures. Its built manifest excluded both activities.
