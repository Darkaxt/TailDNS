...
Version: 1.9. Date: 2026-09-20. Status: **Stage 9 BLOCKED** only on the target Fold's current ADB disconnection. Stages 3–8 are complete; Stages 1 and 2 remain parked only on the explicitly disclosed validation gaps. The Stage 9 implementation, clean validation and public `.6` release are complete, but the required in-place Fold update and post-update real-screen/DNS proof cannot be completed until that device reconnects. Stage 9 closes the Android source-selection interaction defect reproduced on the Galaxy Z Fold 7 and delivers the next signed TailDNS subversion without changing the official Tailscale version component. An unavailable edge-case environment or prohibited automation action must be recorded honestly, but must not stop implementation, deployment or release when the implemented behavior and accessible primary workflows pass.
