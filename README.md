...
**Stages 3 and 4 are complete and Stage 5 integrated reconciliation is active; Stage 1's unavailable network variants and Stage 2's prohibited device-input action are disclosed validation gaps under specification 1.5, not release stops.** Signed Thor candidates demonstrated UI-closed provider changes, fail-closed unsupported input, retained MagicDNS, recovery without reconnecting, autosaving source switches, and observer teardown/recreation. A single cold connect now initializes the backend without a duplicate request. Guard-disabled Wi-Fi loss/return, sleep/wake, repeated starts and live exit-node TUN replacements passed public DNS, local MagicDNS and direct-IP controls. Android's real system Always-on entry now promotes TailDNS to a foreground service, remains alive under ordinary cached-process reclamation, and passes the DNS/control matrix from an absent process. The isolated Windows companion path now passes authenticated set/status/clear, real DoH, MagicDNS, split-route, exit-node, restart-persistence, failure-health and restoration checks without replacing the official service. No public feature release is claimed yet.
