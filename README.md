...
**Stages 3–8 are complete and Stage 7 automatic promotion/release remains active; Stage 1's unavailable network variants and Stage 2's prohibited device-input action are disclosed validation gaps under specification 1.8, not release stops.** The exact integrated Android/core revisions passed the GitHub build and focused suites, isolated signing, independent artifact verification and a final Guard-disabled Thor matrix. UI-closed provider changes failed closed, retained MagicDNS, raised and cleared resolver health, and recovered without reconnecting. Cold app and system Always-on starts, Wi-Fi loss/return, exit-node transitions, observer teardown/recreation and foreground process protection passed. The isolated Windows companion passed authenticated set/status/clear, real DoH, MagicDNS, split-route, exit-node, restart-persistence, failure-health and restoration checks without replacing the official service. The current signed release is [TailDNS 1.103.312-taildns.5](https://github.com/Darkaxt/TailDNS/releases/tag/v1.103.312-taildns.5); it keeps TailDNS activities out of AYN Thor's recent-task removal path without changing the upstream version component. Its public APK was installed on Thor and survived the real AYN **Clear all** action with the same VPN process, Always-on assignment, Control D follow configuration, public resolution and MagicDNS. GitHub detects and validates upstream candidates; the task-attached monitor must promote passing candidates and publish the next independently verified release.
