...
This Android repository owns the specification and Android integration. The [shared Go-core fork](https://github.com/Darkaxt/tailscale/tree/main) contains the resolver path and the independently branded `taildns` Windows CLI. The real Windows workflow is verified with an isolated userspace daemon; the proprietary official Windows GUI is neither copied nor replaced. Production packaging, publication, independent release download verification and an in-place Thor update are recorded in [RELEASE-VALIDATION.md](docs/local-dns-override/RELEASE-VALIDATION.md).
