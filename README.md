...
**Stages 3–8 and 10 are complete; Stage 11 is implementing a transactional Windows in-place upgrade, Stage 9 is blocked only on the disconnected Fold, and Stage 1's unavailable network variants plus Stage 2's prohibited device-input action remain disclosed validation gaps under specification 2.1.** The exact integrated Android/core revisions passed the GitHub build and focused suites, isolated signing, independent artifact verification and the Guard-disabled Thor matrix. UI-closed provider changes failed closed, retained MagicDNS, raised and cleared resolver health, and recovered without reconnecting. Cold app and system Always-on starts, Wi-Fi loss/return, exit-node transitions, observer teardown/recreation and foreground process protection passed. The isolated Windows companion passed authenticated set/status/clear, real DoH, MagicDNS, split-route, exit-node, restart-persistence, failure-health and restoration checks; Stage 11 is converting that verified boundary into a checksum-gated, rollback-capable update of the existing Windows service that preserves login and Tailnet Lock state. The current signed release is [TailDNS 1.103.312+7](https://github.com/Darkaxt/TailDNS/releases/tag/v1.103.312%2B7); it preserves the official upstream version and appends only the standard numeric TailDNS build. Its public APK updated Thor in place with its package data, Always-on assignment, saved Android provider, public resolution and MagicDNS intact. Legacy `-taildns.N` installations already classified as pseudo versions may need this one direct update; subsequent `+N` releases are updater-comparable. GitHub detects and validates upstream candidates; the task-attached monitor promotes passing candidates and publishes the next independently verified release with the same version contract.
...
This Android repository owns the specification and Android integration. The [shared Go-core fork](https://github.com/Darkaxt/tailscale/tree/main) contains the resolver path and the independently branded `taildns` Windows CLI. The proprietary official Windows GUI is neither copied nor replaced; it remains compatibility plumbing while the existing Windows service can be transactionally repointed to the TailDNS daemon without creating another node. Production packaging, publication and independent release verification are recorded in [RELEASE-VALIDATION.md](docs/local-dns-override/RELEASE-VALIDATION.md).
